...
本部分为BasicSR方法的入门部分，主要涉及的有目录解读，核心的工程文件以及其内部运行的逻辑关系，训练，测试和快速推理的流程。这个部分的主要介绍目的是希望读者能够快速的掌握BasicSR的整体结构用于自己开发使用。

\section{目录解读}

基本安装和配置完成之后，首先来看一下BasicSR仓库的基本结构，根据仓库的目录层级，第一部分为仓库的整体概览。这部分主要包括代码基础配置文件和算法核心文件。具体的目录结构如下:

% 文件的大致使用类型分为三类，黑色的为代码基础配置文件，蓝色的为算法基础文件，红色的为算法核心文件。

\newline
\renewcommand*\DTstyle{\ttfamily\textcolor{black}}
\dirtree{%
	.1 \textcolor{black}{BasicSR}.
	.2 .github/workflows\DTcomment{GitHub的自动workflows,比如PyLint和PyPI Publish等}.
	.2 .vscode\DTcomment{VSCode配置，用于统一格式}.
	.2 LICENSE\DTcomment{仓库代码的LICENSE和Acknowledgement}.
	.2 assets\DTcomment{仓库展示图片}. 
	.2 \textcolor{red}{basicsr}\DTcomment{BasicSR核心代码包}.
	.2 colab\DTcomment{Google Colab的Notebook, 提供方便的inference demo}.
	...
	.2 \textcolor{red}{options}\DTcomment{训练和测试配置文件}.
	.2 \textcolor{red}{scripts}\DTcomment{功能脚本，包含数据集制作，指标测试和数据集下载等}.
	.2 \textcolor{red}{tests}\DTcomment{简易代码单元测试}.
	.2 .gitignore\DTcomment{Git忽略文件配置}.
	.2 .pre-commit-config.yaml\DTcomment{Pre-commit Hook的配置文件}.
	.2 INSTALL.md\DTcomment{安装指南}.
	.2 MANIFEST.in\DTcomment{basicsr发布所需的相关文件}.
	.2 README.md\DTcomment{说明文档}.
	.2 README\_CN.md\DTcomment{说明文档中文版}.
	.2 VERSION\DTcomment{版本文件}.
	.2 requirements.txt\DTcomment{package依赖文件}.
	.2 setup.cfg\DTcomment{格式配置文件（flake8,yapf和isort）}.
	.2 \textcolor{red}{setup.py}\DTcomment{安装配置文件}.
}

在BasicSR仓库中，核心代码包为basicsr，这个部分主要为深度学习模型常用的代码文件，比如网络结构，损失函数和数据加载等，具体目录如下：
\dirtree{%

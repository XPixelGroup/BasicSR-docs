...
\begin{exampleBox}[righthand ratio=0.00, sidebyside, sidebyside align=center, lower separated=false]{数据读取相关配置}
    \begin{itemize}
        ...
        \item meta\_info\_file：细节请参看章节\ref{data_preparation:meta_file}：\nameref{data_preparation:meta_file}
        \item io\_backend：读取数据的方式，细节请参看章节\ref{data_preparation:common_use}：\nameref{data_preparation:common_use}
...
\begin{exampleBox}[righthand ratio=0.00, sidebyside, sidebyside align=center, lower separated=false]{validation 相关配置}
    \begin{itemize}
        \item 关于 metrics 的介绍，请参考章节\ref{chapter:metrics}：\nameref{chapter:metrics}
...
\begin{exampleBox}[righthand ratio=0.00, sidebyside, sidebyside align=center, lower separated=false]{训练日志相关配置}
    \begin{itemize}
        \item 关于 wandb，目前 wandb 只是同步 tensorboard 的内容, 因此要使用 wandb, 必须也同时使用 tensorboard。更多关于 wandb，参见章节\ref{code_structure:wandb_record}：\nameref{code_structure:wandb_record}
    \end{itemize}
\end{exampleBox}
...
\begin{note} % ---------------- Note block ---------------- %
    meta\_info 的 txt 文件是可以根据自己的需要创建的。BasicSR 提供了一些约定的 meta\_info 文件，具体参见章节\ref{data_preparation:meta_file}：\nameref{data_preparation:meta_file}。
\end{note}
...
\begin{exampleBox}[righthand ratio=0.00, sidebyside, sidebyside align=center, lower separated=false]{数据读取格式}
    ...
    \end{enumerate}
    详见参见 File Client 说明章节\ref{data_preparation:file_client}：\nameref{data_preparation:file_client}。
\end{exampleBox}
...
\begin{note} % ---------------- Note block ---------------- %
    如果训练过程中 IO 读取慢，推荐使用 LMDB 数据格式加速，详见章节\ref{data_preparation:lmdb}：\nameref{data_preparation:lmdb}。
\end{note}
...
\begin{enumerate}
    \item 被添加到 log 文件 (第\ref{code_structure:log_record}小节：\nameref{code_structure:log_record}) 中的值，会被自动添加到 tb\_logger 中， 比如损失函数 (第\ref{code_structure:loss_log}小节：\nameref{code_structure:loss_log})
